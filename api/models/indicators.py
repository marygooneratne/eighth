...
class Indicators:
    ...
    def ema(prices, period, type=''):
        ...
        exponential_ma[period - 1] = Indicators.calc_ema(base_sma,
                                                         prices[period-1], multiplier)
        l = len(prices)
        for i, p in enumerate(prices):
            if i - period < 0:
                continue
            ma = 0
            exponential_ma[i] = Indicators.calc_ema(exponential_ma[i-1],
                                                    prices[i],
                                                    multiplier)
    ...
    def macd_indicator(prices, slow_period, fast_period):
        ...
        for i, macd_val in enumerate(macd_vals):
            ...
            macd_ind[i] = Indicators.gen_macd_ind_lbl(
    ...
    def gen_macd_ind_lbl(macd_val_0, macd_val_1, macd_ema_0, macd_ema_1):
        cross = Indicators.check_intersection(
    ...
    def calc_average_true_range(true_ranges, period=10):
        ...
        for i, tr in enumerate(true_ranges):
            if(i+1 == len(true_ranges)):
                atr[i+1] = (prevatr * (period - 1) +
                            true_ranges[i+1]) / period
            prevatr = atr[i+1]
    ...
    def kst(prices):
        ...
        for i, p in enumerate(rcma_4):
            kst_vals[i] = rcma_1[i] + rcma_2[i] * \
                2 + rcma_3[i] * 3 + rcma_4[i] * 4
        return kst_vals
